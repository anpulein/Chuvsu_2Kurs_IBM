...
; КЛАВИАТУРЫ)
 ...
 Int09Hand proc
    ...
    mov   AL,ES:[418h]; нажатие <Shift Right + CapsLock>
    and   AL,01000001b      ; 6 - байт CapsLock
    cmp   AL,01000001b     ; 0 -байт Shift Right
    je    Cont        ;
...
;-----------------------------------D------------------------------------------;
		D1:
			in AL,60h 
			cmp AL,20h ; Скан код нажатия буквы D
			je D2 ; Если D идем на проверку отжатия
			jmp D1 ; Если не D то начинаем все заново
			
		D2:
			in AL,60h 
			cmp AL,0A0h ; Скан код отжатия буквы D
			je M1 ; Если D отжали,то идем дальше
			jmp D2 ; Если кнопка еще не отжата ждем отжатия
			
;-----------------------------------M------------------------------------------;
			
		M1:
			in AL,60h 
			cmp AL,32h ; Скан код нажатия буквы M
			je M2 ; Если M идем на проверку отжатия
			cmp AL, 0A0h ; Если не M идем на проверку того что пользователь не нажимал
			je M1 
			jmp D1 
			
		M2:
			in AL,60h 
			cmp AL,0B2h ; Скан код отжатия буквы M
			je I1 ; Если M отжали,то идем дальше
			jmp M2 ; Если кнопка еще не отжата ждем отжатия
			
;-----------------------------------I------------------------------------------;
			
		I1:
			in AL,60h 
			cmp AL,17h ; Скан код нажатия буквы I
			je I2 ; Если I идем на проверку отжатия
			cmp AL, 0B2h ; Если не I идем на проверку того что пользователь не нажимал
			je I1 
			jmp M1 
			
		I2:
			in AL,60h 
			cmp AL,97h ; Скан код отжатия буквы I
			je T1 ; Если I отжали,то идем дальше 
			jmp I2 ; Если кнопка еще не отжата ждем отжатия
			
;-----------------------------------T------------------------------------------;
			
		T1:
			in AL,60h 
			cmp AL,14h ; Скан код нажатия буквы T
			je T2 ; Если T идем на проверку отжатия
			cmp AL, 97h ; Если не T идем на проверку того что пользователь не нажимал
			je T1 
			jmp I1 
			
		T2:
			in AL,60h 
			cmp AL,94h ; Скан код отжатия буквы T
			je R1 ; Если T отжали,то идем дальше 
			jmp T2 ; Если кнопка еще не отжата ждем отжатия
			
;-----------------------------------R------------------------------------------;

		R1:
			in AL,60h 
			cmp AL,13h ; Скан код нажатия буквы R
			je R2 ; Если R идем на проверку отжатия
			cmp AL, 94h ; Если не R идем на проверку того что пользователь не нажимал
			je R1 
			jmp T1 
			
		R2:
			in AL,60h 
			cmp AL,93h ; Скан код отжатия буквы R
			je Y1 ; Если R отжали,то идем дальше 
			jmp R2 ; Если кнопка еще не отжата ждем отжатия

;-----------------------------------Y------------------------------------------;
   
		Y1:
			in AL,60h 
			cmp AL,15h ; Скан код нажатия буквы Y
			je Y2 ; Если Y идем на проверку отжатия
			cmp AL, 93h ; Если не Y идем на проверку того что пользователь не нажимал
			je Y1 
			jmp R1 
			
		Y2:
			in AL,60h 
			cmp AL,95h ; Скан код отжатия буквы Y
			je Konec ; Если Y отжали,то идем дальше 
			jmp Y2 ; Если кнопка еще не отжата ждем отжатия
   ...
   Exit009:
     ...
     mov AL,ES:[418h];│<Control+Alt> по адресу
     and AL,10111110b;│0000h:0417h и флаги
     mov ES:[418h],AL;│<LeftControl+LeftAlt>
	 mov AL,ES:[417h];│<Control+Alt> по адресу
     and AL,11011110b;│0000h:0417h и флаги
 ...
 InstMsg     db 'The LOCKER program is installed. To withdraw'
             db 'use key /u',CR,LF
             db 'To "lock", press <Shift Right+CapsLock+L>',CR,LF
             db 'To "unlock", press DMITRY'
             db CR,LF,'$'
